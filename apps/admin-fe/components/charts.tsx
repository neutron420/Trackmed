        ...
        {data.map((item, index) => (
          <div key={index} className="space-y-1">
            <div className="flex items-center justify-between text-xs">
              <span className="text-slate-600 truncate max-w-[120px]">{item.label}</span>
...
export function SimpleDonutChart({ data, size = 140, showTotal = false, showLegend = true }: DonutChartProps) {
  ...
  if (total === 0) {
    return (
      <div className={`flex items-center ${showLegend ? 'justify-center gap-6' : 'justify-center'}`}>
...
const colorMap = {
  ...
  red: { main: "#ef4444", gradient: "rgba(239, 68, 68, 0.2)" },
};

export function AreaChart({ data, labels, height = 160, color = "emerald", showGrid = true, showPoints = true }: AreaChartProps) {
