...
export function NotificationBell() {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const response = await fetch(
        `${apiUrl}/api/notification?limit=10`,
        {
          headers: {
      ...
      if (response.ok) {
        ...
        console.warn("Notification API: Unauthorized - token may be expired");
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      fetchUnreadCount();
      fetchNotifications(); // Also refresh notifications periodically
